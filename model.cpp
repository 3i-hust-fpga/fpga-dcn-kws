...
static void gen_output(hls::stream<DTYPE> &in, hls::stream<axis_data> &out, int size) {
    axis_data tmp;
    DTYPE val;
...
static void model(DTYPE *in1, DTYPE *in2, hls::stream<axis_data> &out) {
...
#pragma HLS DATAFLOW
	...
	gen_output(stream_out, out, OUTPUT_SIZE);
}
...
#pragma HLS PIPELINE
    ...
    model(array_1, array_2, out);

}

void model_test(DTYPE in1[CONV_IN_SIZE_0 * CONV_IN_SIZE_1], DTYPE in2[CONV_IN_SIZE_0 * CONV_IN_SIZE_1], hls::stream<axis_data> &out) {
